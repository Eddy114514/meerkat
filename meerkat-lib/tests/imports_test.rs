...
/// A private scratch directory for a test that resolves imports from disk.
...
/// The service names in `unified_ast`, in order.
fn service_order(node: &meerkat_lib::runtime::Node) -> Vec<String> {
    node.unified_ast
        .iter()
...
async fn test_imports_precede_local_program_in_unified_ast() {
    let dir = unique_temp_dir("import_order");
...
async fn test_transitive_imports_are_dependency_ordered() {
    let dir = unique_temp_dir("nested_import");
...
async fn test_on_node_startup_also_orders_imports_first() {
    let dir = unique_temp_dir("startup_order");
    std::fs::write(
        dir.join("dep.mkt"),
        "service dep {\n    var x = 0;\n    pub def y = x + 1;\n}\n",
    )
